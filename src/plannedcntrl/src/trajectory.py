...
def plan_curved_trajectory(target_position):
    ...
    tran = np.array([[np.cos(yaw), -1 * np.sin(yaw), 0, x1], [np.sin(yaw), np.cos(yaw), 0, y1], [0, 0, 1, 0], [0, 0, 0, 1]])
    res = tran @ np.array([target_position[0], target_position[1], 0, 1])
    x2 = res[0]  ## TODO: how would you get x2 from our target position? remember this is relative to x1 
    y2 = res[1] ## TODO: how would you get x2 from our target position? remember this is relative to x1 
